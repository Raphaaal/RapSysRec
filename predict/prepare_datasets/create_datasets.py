...
def write_artist_specific_set(artist_urn, path, driver):
    with driver.session() as session:
        ...
        # Get negative links within 1..3 hops (missing)
        result = session.run(
            """
...
if __name__ == '__main__':
    ...
    # train_set = get_train_set(max_links=20000, batch_size=3, target_year=2020, driver=driver)
    # train_set.to_csv('train_set.csv', index=False)
    # logger.info("Ended training set creation")

    # Build test dataset
    # logger.info("Started testing set creation")
    # test_set = get_test_set(max_links=20000, density=0.01, batch_size=3, target_year=2020, driver=driver)
    # test_set.to_csv('test_set.csv', index=False)
    # logger.info("Ended testing set creation")

    # Build validation dataset
    # logger.info("Started validation set creation")
    # validation_set = get_test_set(max_links=20000, density=0.01, batch_size=3, target_year=2020, driver=driver)
    ...
    # All pairs in the 1..3rd hop of the considered artist
    ...
    write_artist_specific_set(artist_urn='6qFt3TjvxMt77YGsktWG8Z', path='artist_set.csv', driver=driver)
